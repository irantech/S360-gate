    ...
    <div class="inner-avlbl-itm">
        ...
        <div class="SelectTicket" data-ticket-detail='<?php echo json_encode($ticketTrain,256|64); ?>' direction="<?php echo ($ticketTrain['TypeRoute'] == 'return') ? 'return':'dept' ?>">
            ...
            if(functions::compareTimeTrainForShowList($ticketTrain['ExitTime'],substr($ticketTrain['MoveDate'], 0, 10),substr($ticketTrain['ExitDate'], 0, 10))) {
                ...
                if(intval($ticketTrain['ChildCount']+$ticketTrain['InfantsCount']+$ticketTrain['AdultCount']) <= intval($ticketTrain['CompartmentCapicity'])){
                    if ($ticketTrain['Counting'] <= 0 || ($ticketTrain['Counting'] < $ticketTrain['AdultCount'])) {
                        ?>
                    ...
                    <?php } elseif ($ticketTrain['IsCompartment'] != '1' && $params['Coupe'] == '1') {
                        ?>
                      <span class="no-coupe-train"><?php echo functions::Xmlinformation('TrainNotCoupe') ?></span>
                    <?php } elseif ($ticketTrain['CompartmentCapicity'] > $ticketTrain['Counting'] && $ticketTrain['IsCompartment'] == 1 && $params['Coupe'] == 1) { ?>
                      <span  class="no-coupe-train"><?php echo functions::Xmlinformation('LostCapacityOfOneCoupe') ?></span>
                    <?php } else { ?>
                      <a class="international-available-btn site-bg-main-color nextStepclassApiTrain" id="nextStep_<?php echo $key ?>" keyId="<?php echo $key ?>">
                  ...
                  ?>
                  <a class="international-available-btn site-main-button-color-hover flight-false">غیر قابل رزرو</a>
                    <?php
                }
            ...
            <input type="hidden" value="<?php echo $this->getController('baseCompanyBus')->getCompanyTrainById($ticketTrain['Owner'])?>" class="CompanyName">
            ...
            <input type="hidden" value="<?php echo $ticketTrain['AdultCount']?>" id="adult">
            <input type="hidden" value="<?php echo $ticketTrain['ChildCount']?>" id="child">
            <input type="hidden" value="<?php echo $ticketTrain['InfantsCount']?>" id="infant">
            <input type="hidden" value="<?php echo $ticketTrain['serviceSessionId'] ?>" id="serviceSessionId">
            <input type="hidden" value="<?php echo $ticketTrain['RequestNumber'] ?>" id="code">
