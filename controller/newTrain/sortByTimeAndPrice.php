...
<div id="sort_train" class="sorting2">
    <div class="sorting-inner date-change iranL prev <?php if($params['directions'] == 'TwoWay'){echo 'disable_two' ;}  ?>">
        <a class="prev-date"
           href="<?php if ($params['directions'] == 'OneWay' && $this->indate($params['DepartureDate']) == true) {
               echo ROOT_ADDRESS . '/trainResult/' . $params['DepartureCityCode'] . '-' . $params['ArrivalCityCode'] . '/' . $this->DatePrev($params['DepartureDate']) . '/' . $params['TypeWagon'] . '/' . $params['AdultCount'] . '-' . $params['ChildCount'] . '-' . $params['InfantCount'] . '/' . $params['Coupe'];
           } else {
               ?> javascript:<?php
           } ?>">
    ...
    <div class="sorting-inner date-change iranL next <?php if($params['directions'] == 'TwoWay'){echo 'disable_two' ;}  ?>">
        <a class="next-date"
           href="<?php if ($params['directions'] == 'OneWay' && $this->indate($params['DepartureDate']) == true) {
               echo ROOT_ADDRESS . '/trainResult/' . $params['DepartureCityCode'] . '-' . $params['ArrivalCityCode'] . '/' . $this->DateNext($params['DepartureDate']) . '/' . $params['TypeWagon'] . '/' . $params['AdultCount'] . '-' . $params['ChildCount'] . '-' . $params['InfantCount'] . '/' . $params['Coupe'];
           } else {
               ?> javascript:<?php
           } ?>">
