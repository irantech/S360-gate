...
class boxCheck
{
    ...
    public function createPdfContent($param)
    {
        if (TYPE_ADMIN == '1') {
            ...
            $Model = new ModelBase();
            $sql = "SELECT * , "
                . " (SELECT count(id)   FROM  report_tb WHERE request_number='{$param}' ) AS CountTicket "
                . "  FROM  report_tb  WHERE request_number='{$param}'";
        } else {
            Load::autoload('Model');
            $Model = new Model();

            $sql = "SELECT * , "
                . " (SELECT count(id)   FROM  book_local_tb WHERE request_number='{$param}' ) AS CountTicket "
                . "  FROM  book_local_tb  WHERE request_number='{$param}'";
        }
        ...
        $FlightDate =functions::convertDateFlight($res['date_flight']);
                          ...
                          <tr dir="rtl" align="right"  style="margin:10px; ">
                        	<td align="right" dir="rtl">
                              <table width="1000" border="0" cellspacing="0" cellpadding="0">
                            	  <tr>
                            		<td height="30" width="200" id="print_sandogh" class="lh50"><span style="font-size:15px; font-family: BKoodakBold;" class="lh50">'.functions::Xmlinformation("Invoicenumber").' :</span> <span style="font-size:15px">' . $res['factor_number'] . '</span></td>
                            		...
                            		<td align="right" width="300" id="print_sandogh" dir="rtl" >
                            		    <div style="direction:rtl; text-align:right;"><span style="font-size:15px" class="dir-rtl">'.functions::Xmlinformation("Numberreservation").':</span><span dir="rtl" style="font-size:15px">' . $res['request_number'] . '</span></div>
                            		</td>
                            	  </tr>
            	              </table>
                            </td>
...
//            if ($i == 1) {
                $passenger_name =  $res['passenger_name_en'] ? $res['passenger_name_en'] : $res['passenger_name'];
                $passenger_family =  $res['passenger_family_en'] ? $res['passenger_family_en'] : $res['passenger_family'];
                $prinBoxCheck .= $type_pay . ' ' . ':' . ' ' . $passenger_name.' '.$passenger_family . '</td>';
...
//            }
            $prinBoxCheck .= '</tr>
                          ...
                          <tr>
            	            <td height="30" id="print_sandogh" dir="rtl" style="font-size:18px">
                              <span style="font-size:18px">  '.functions::Xmlinformation("Phonenumber").' :</span> ' . ($i !=1 ? $res['member_mobile'] : $res['mobile_buyer']) . '
            		        </td>
                          </tr>
                          <tr>
            ...
            if ($res['type_app'] == 'reservationBus'){
                $prinBoxCheck .= '
            	            <td style="font-size:18px" height="30" id="print_sandogh" dir="rtl">
            	'.functions::Xmlinformation("CashierReceipts").' '.$res['flight_number'] . ' - ' . $res['desti_city'] . ' ' . functions::Xmlinformation("ToReservationNumber") . ' ' . $res['request_number'] . ' ' . '  '.functions::Xmlinformation("ToDate").'  <span style="direction:rtl">' . ($i!=1 ? $sodorDate : $FlightDate) . '</span> </span>  
                            </td>';

            } else {
                $prinBoxCheck .= '
            	            <td style="font-size:18px" height="30" id="print_sandogh" dir="rtl">
            '.functions::Xmlinformation("ForExportServices").' ' . $res['CountTicket'] . ' '.functions::Xmlinformation("NumberOfFlights").' ' . $res['origin_city'] . ' '.functions::Xmlinformation("On").' ' . ' ' . $res['desti_city'] . ' ' . functions::Xmlinformation("ToReservationNumber") . ' ' . $res['request_number'] . ' ' . '  '.functions::Xmlinformation("ToDate").'  <span style="direction:rtl">' . ($i!=1 ? $sodorDate : $FlightDate) . '</span> </span>  
                            </td>';
            }
