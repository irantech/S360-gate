...
//        @ini_set('display_errors', 'on');
    ...
    class petroTools extends mainPage {
        ...
        public function dataFastSearchInternalFlight($params)
        {
            ...
            // ----------------------------------------------------------
            // 🔥 NEW: override top visible cities (THR → MRX → PGU)
            // ----------------------------------------------------------
            $priorityOverride = ['THR', 'MRX', 'PGU'];

            $sorted_flights = [];

            // first push override cities
            foreach ($priorityOverride as $code) {
                foreach ($array_cities['cities_flight'] as $item) {
                    if ($item['main']['Departure_Code'] == $code) {
                        $sorted_flights[] = $item;
                    }
                }
            }

            // add all remaining items except overridden ones
            foreach ($array_cities['cities_flight'] as $item) {
                if (!in_array($item['main']['Departure_Code'], $priorityOverride)) {
                    $sorted_flights[] = $item;
                }
            }
            ...
            // replace with sorted
            $array_cities['cities_flight'] = $sorted_flights;
