  ...
  <div>
    <div class="parent-data-search-box-flight">
      <div class="data-flight" :class='scroll_down' >
          <div class="box-data-flight" v-show='!detail_search_box && mobileHeaderSearchBox' @click='windowWidth > 576 ? showDetailSearchBox() : null'>
          ...
          <div class="new_search_box" v-show='detail_search_box'>
            <div class="tab-content" id="searchBoxContentFlight">
                ...
                <div id="internal_flight" class="d_flex flex-wrap internal-flight-js">
                  <form method="post" class="d_contents" target="_blank" id="internal_flight_form" name="internal_flight_form">
                      ...
                      <originDestination :stored_origin_cities='stored_origin_cities'
                                         ...
                                         type="'domestic'"/>
                      <date type="'domestic'"/>
                      <passengers type="'domestic'"/>
                      <approve :searchModule="'domestic-flight'"/>
                  </form>
                </div>
              </div>
            </div>
...
<script>
  ...
  import originDestination from '../global/searchBox/domestic/originDestination'
  ...
  export default {
    ...
    methods : {
      ...
      setSearchedStorage() {
        ...
        if(storage_searched_cities == null || storage_searched_cities == "null" || Object.keys(JSON.parse(storage_searched_cities)).length == 0 ) {
          ...
          console.log('ddd' , this.dataSearch.name_departure)
          storage_origin_cities.push({
            Departure_Code : this.iata_origin  ,
            [`Departure_City${this.getLang().charAt(0).toUpperCase() + this.getLang().slice(1)}`] : this.dataSearch.name_departure
          })
          storage_arrival_cities.push({
            Departure_Code : this.iata_arrival  ,
            [`Departure_City${this.getLang().charAt(0).toUpperCase() + this.getLang().slice(1)}`] : this.dataSearch.name_arrival
          })
    ...
    computed :{
      ...
      mobileHeaderSearchBox() {
        return this.$store.state.mobileHeaderSearchBox
      }
    },
