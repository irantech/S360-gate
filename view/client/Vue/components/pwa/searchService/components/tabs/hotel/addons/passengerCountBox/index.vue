  ...
  <div class="select position-unset inp-s-num adt box-of-count-nafar">
    <div
      ...
      <span class="text-count-nafar">
        {{ form[strategy].passengers_number.rooms.length }} {{ useXmltag('Room')}}
      </span>
    ...
    <div
      ...
      <div
        ...
        <div
          ...
          <span class="font-weight-bold m-0 font-19">{{ useXmltag('Room')}} {{ key + 1 }}</span>
        ...
        <div class="d-flex w-100 p-0 flex-wrap">
          <button class="addroom" @click="addRoom">{{ useXmltag('Addroom')}}</button>
        </div>
      ...
      if (index_count < max_count) {
        ...
        if (show_error) {
          this.$swal({
            ...
            timer: 4000,
            title: useXmltag('maximum')+max_count,
          })
        }
        return false
      }
