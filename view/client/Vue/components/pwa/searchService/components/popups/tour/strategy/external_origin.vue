      ...
      <div
        ...
        <div class="form-group origin_start">
          <input
            ...
            type="text"
            class="form-control"
            :placeholder="`${useXmltag('Origincountry')}`" />
        </div>
      </div>
      <div class="col-lg-12 col-md-12 col-12 col_search search_col">
        <div class="form-group">
          <input
            ...
            :placeholder="`${useXmltag('Origincity')}`"/>
          <app-loading-spinner
      ...
      <ul>
        ...
        <li
          ...
          <button
            ...
            ">
            {{getLang() == 'fa' ?  item.city.title : item.city.title_en }}
          </button>
        </li>
      </ul>
    </div>
...
export default {
  ...
  data() {
    return {
      allItem : {
        "city": { "title": "همه مبداها", "title_en": "all", "value": "all" }
      },
      form: {
        origin: {
          city: null,
          country:  this.getLang() == 'fa' ? "ایران" : "iran",
        },
        destination: {
          city: null,
          country: null,
  ...
  computed: {
    ...
    city_list() {
      console.log('dddddddddddd' ,this.current_panel)
  ...
  methods: {
    async selectCity(item) {
      ...
      this.form[this.refillable.define.index][index_name] = this.getLang() == 'fa' ?  item.city.title : item.city.title_en
