<template>
   <div v-if="price_cip_filter && price_cip_filter.length" class="parent-price-basis">
      <div class="accordion_cip" id="price-basis">
...
export default {
   ...
   computed: {
      min_price_props() {
         if (this.price_cip_filter.length) return this.price_cip_filter[0]
      },
      max_price_props() {
         if (this.price_cip_filter.length) return this.price_cip_filter[this.price_cip_filter.length - 1]
      },
   },
   watch: {
      price_cip_filter: {
   ...
   methods: {
      setPriceCipFilter() {
         if (this.price_cip_filter && this.price_cip_filter.length > 0) {
            this.value_price = [this.price_cip_filter[0], this.price_cip_filter[this.price_cip_filter.length - 1]]
         }
      },
      priceRangeSlider(value) {
         let check_min_max_price = true
         if (
            parseInt(value[0]) != parseInt(this.price_cip_filter[0]) ||
            parseInt(value[1]) != parseInt(this.price_cip_filter[this.price_cip_filter.length - 1])
