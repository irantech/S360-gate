<template>
  <div>
    ...
    <div class="radios" v-if="this.$store.state.pwa_page.active_service == null || (this.$store.state.pwa_page.active_service && this.$store.state.pwa_page.active_service.type == null)">
      <div class="switch mt-2" v-if="service.which == 'both'">
        <input
          autocomplete="off"
          type="radio"
          class="switch-input"
    ...
    <div v-if="index_data.strategy === 'local' && (service.which == 'both' || service.which == 'Local')" id="flight_dakheli" class="row">
      <strategy-local
        rel="awdawdawd"
        :main_url="main_url"
...
export default {
  ...
  data() {
    return {
      index_data: {
        key: "flight",
        title:useXmltag('Flight').toLowerCase(),
        strategy: "local",
      },
      form: {
        local: {
