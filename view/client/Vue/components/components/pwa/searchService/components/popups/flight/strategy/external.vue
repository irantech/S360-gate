      ...
      <ul>
        ...
        <li
          ...
          <button
            ...
            ">
            {{ getLang() == 'fa' ?  city.country :  city.country_en }} - {{ getLang() == 'fa' ?  city.title :  city.title_en}} ({{ getLang() == 'fa' ? city.airport : city.airport_en  }})
          </button>
        </li>
      </ul>
    </div>
...
export default {
  ...
  computed: {
    ...
    city_list() {
      if (this.current_panel.searched_cities === null) {
        console.log("null", this.current_panel.default_cities)
        return this.current_panel.default_cities
      } else {
        console.log("not null", this.current_panel.searched_cities)
  ...
  methods: {
    async selectCity(city) {
      ...
      if(this.getLang() == 'fa') {
        this.form[this.refillable.define] = city.title + "(" + city.airport + ")"
      }else{
        this.form[this.refillable.define] = city.title_en + "(" + city.airport_en + ")"
      }
    ...
    async searchCity(searchable) {
      ...
      if (!searchable) {
        ...
        console.log("this.refillable.define")
        await this.$store.commit("setPwaData", {
          index: this.index_data.key,
          data: {
