<template>
  <form
    ...
    target="_blank">
    <div class="row">
      <div class="col-md-12 position-unset col-xs-12 col-sm-12 d-ceckboxs">
        <div class="cntr">
          <label
          ...
          <label
            ...
            <span>{{ useXmltag('Twoway')}} </span>
          </label>
        </div>
      </div>
      <div
        class="col-lg-2 col-md-12 col-sm-12 col-12 col_search search_col col_with_route">
        <div class="form-group">
          <div class="form-group">
            <div
              ...
              <span v-if="form[index_data.strategy].origin.title || form[index_data.strategy].origin.title_en">
                {{getLang() == 'fa' ?   form[index_data.strategy].origin.title : form[index_data.strategy].origin.title_en }}
              </span>
              <span v-else>{{ useXmltag('OriginCityAirPlane2')}}</span>
            </div>
          </div>
        </div>
      ...
      <div class="col-lg-2 col-md-12 col-sm-12 col-12 col_search search_col">
        <div class="form-group">
          <div class="form-group">
            <div
              @click="callOpenPanel()"
              class="fake-input align-content-center font-11 text-muted align-items-center d-flex form-control inp_flight p-3-5">
              <span class="far fa-location" :class="`${getLang() == 'en' ? 'mr-2' : 'ml-2'}`"></span>
              <span v-if="form[index_data.strategy].destination.title || form[index_data.strategy].destination.title_en">
                {{ getLang() == 'fa' ?  form[index_data.strategy].destination.title : form[index_data.strategy].destination.title_en }}
              </span>
              <span v-else>{{ useXmltag('DestinationCityAirPlane2')}}</span>
            </div>
          </div>
        </div>
      </div>
      <div class="col-lg-4 col-md-12 col-sm-12 col-12 col_search date_search">
        <div class="form-group">
          <template>
            <date_picker
              popover
              ref="datepicker"
              :placeholder="`${useXmltag('DepartureDate')}`"
        ...
        <div class="form-group">
          <template>
            <date_picker
              popover
              :placeholder="`${useXmltag('Datewentback')}`"
              ref='return_date'
...
export default {
  ...
  created() {
    console.log('main_url ,' , this.main_url)
  },
  data() {
    return {
      view_only_form: {
