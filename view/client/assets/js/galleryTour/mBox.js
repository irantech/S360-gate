...
$(function($) {
	...
	MBox.prototype = {
		...
		build: function() {
			...
			if(this.imagesCount > 1) {
				...
				$('<i/>', {
					'class': 'fas fa-light fa-chevron-left point mBox-previous float-left hide'
				}).appendTo(navigationArrows);

				$('<i/>', {
					'class': 'fas fa-light fa-chevron-right point mBox-next float-right hide'
				}).appendTo(navigationArrows);
				...
				if(this.settings.displayThumbnails) {
					...
					var width = 90 / this.settings.imagesPerPage + '%'; // Calculate width.
			...
			}).append('<span><i class="fa-light fa-xmark"></i></span>').prependTo(container);
		...
		createContainer: function() {
			...
			// Remove any existing mBox and overlay to prevent duplicates
			$('#mBox').remove();
			$('.body-overlay').remove();
		...
		createNavigation: function() {
			...
			var navigationCount = this.mBox.find('p.mBox-navigation-txt');
			...
			if(this.imagesCount > 1) {
				...
				var thumbnailPagination = this.mBox.find('p.mBox-thumbnail-pagination');
				...
				if((this.currentImageNo + 1) != this.imagesCount) {
					this.mBox.find('i.mBox-next').removeClass('hide');
				}
				if(this.currentImageNo != 0) {
					this.mBox.find('i.mBox-previous').removeClass('hide');
				}
				...
				if(this.imagesCount > this.settings.imagesPerPage) {
					...
					if(Math.floor(this.currentImageNo / this.settings.imagesPerPage) !== this.currentPageNo) {
						...
						this.mBox.find('.mBox-thumbnail-pagination span').removeClass('active');
						this.mBox.find('.mBox-thumbnail-pagination span[data-page-number = ' + left + ']').addClass('active');

						this.currentPageNo = left; // Set new page number before multiplying by 100.

						left = left * 100;

						this.mBox.find('.mBox-images').css('left', left + '%'); // Animation not needed here.
					}
				}
		...
		navigationUpdate: function(no) {
			...
			this.mBox.find('.mBox-current-image-no').text(no + 1);

			if(this.settings.displayTitle == true) {

				var title = ($(this.currentImage).attr('title')) ? $(this.currentImage).attr('title') : '';

				this.mBox.find('.mBox-current-image-title').text(title);

			}

			// Update navigation arrows.

			if((no + 1) != this.imagesCount) this.mBox.find('i.mBox-next').removeClass('hide');
			else this.mBox.find('i.mBox-next').addClass('hide');
			if(no != 0) this.mBox.find('i.mBox-previous').removeClass('hide');
			else this.mBox.find('i.mBox-previous').addClass('hide');
			...
			if(this.imagesCount > this.settings.imagesPerPage) {
				...
				if(Math.floor(this.currentImageNo / this.settings.imagesPerPage) !== this.currentPageNo) {
					...
					this.mBox.find('.mBox-images').animate({'left': left + '%'}, 'slow');
		...
		updateVars: function() {
			...
			this.currentImage = this.mBox.find('.mBox-current img'); // current_image var points now to the featured image.

			//if(this.imagesCount > 1) this.images = this.mBox.find('.mBox-images img'); // Images array points to the thumbnails right now.
		...
		addEventListeners: function() {
			...
			// Close events
			this.mBox.find('button.mBox-close span').on('click', $.proxy(function() {
				this.close();
			}, this));
			...
			this.mBox.find('img.mBox-thumbnail').on('click', $.proxy(function(e) {
			...
			this.mBox.find('i.mBox-next').on('click', $.proxy(function(e) {

				this.next();

			}, this));

			this.mBox.find('i.mBox-previous').on('click', $.proxy(function(e) {
			...
			this.mBox.find('.mBox-thumbnail-pagination span').on('click', $.proxy(function(e) {
			...
			if(this.imagesCount > 1 && this.settings.displayNavigation == true) {
				...
				this.mBox.find('.mBox-current').on('mouseenter', $.proxy(function(e) {
				...
				this.mBox.find('.mBox-current').on('mouseleave', $.proxy(function(e) {
	...
	$.fn.mBox = function(options) {
		...
		$(this).on('click.mBox', function(e) {
			e.stopPropagation();
			var clicked = e.target;

			// اگر روی یک تصویر کلیک شد
			if (clicked.tagName === 'IMG') {
				new MBox(this, clicked, options);
			}
		});

		// اضافه کردن عملکرد برای کلیک روی div
		$('.more-image').on('click', () => {
			var firstImage = $(this).find('img').first(); // پیدا کردن اولین تصویر
			if (firstImage.length) {
				new MBox(this, firstImage[0], options); // باز کردن MBox با اولین تصویر
			}
		});
