$(document).ready(function () {
    ...
    document.getElementById("cityOrigin")
       .onclick = ()=> openPopularBusCities("cityOrigin");

    document.getElementById("cityDestination")
       .onclick = ()=> openPopularBusCities("cityDestination");
...
async function reserveBusTicket(busCode, sourceCode, checkLogin = true, _this = null) {
    ...
    if (checkLogin) {
        $.post(amadeusPath + 'hotel_ajax.php',
          ...
          function(data) {
            console.log(data.indexOf('successLoginHotel'))
...
async function reserveBusTicketWithoutRegister(description_only=false) {
    ...
    await $.post(amadeusPath + 'bus_ajax.php',
        ...
        function (data) {
            ...
            parseData=JSON.parse(dataArray[1]);
            ...
            if (data.indexOf('success') > -1) {
                ...
                if(description_only){
                    ...
                    console.log('parseData.refundRules',parseData.refundRules)
...
function checkInfoPassenger(currentDate) {
    ...
    if($('[name="mobilePhone1"]').length > 0){
        ...
        if (!mobregqx.test($('[name="mobilePhone1"]').val())) {
            console.log('phone=>'+mobregqx.test(mobile_buyer));
        ...
        if (!mobregqx.test(mobile_buyer)) {
            console.log('Mobile=>'+mobregqx.test(mobile_buyer));
            $("#messageInfo").css('color', 'red').html(useXmltag("MobileNumberIncorrect"));
            error = 1;
        }
    }
    ...
    if ($("#email1").length > 0) {
        ...
        if (email_buyer == '' && mobile_buyer != '') {
            ...
            if(mobile_buyer == ''){
                console.log('????=>'+mobregqx.test(mobile_buyer));
                $("#messageInfo").css('color', 'red').html(useXmltag("MobileNumberIncorrect"));
                error = 1;
            }
        }
