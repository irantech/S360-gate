    ...
    <div class="row bg-title">
        <div class="col-lg-6 col-sm-6 col-md-6 col-xs-12">
            <ol class="breadcrumb FloatRight">
                ...
                <li class="active">تکرار مرحله سرچ بلیط {if $smarty.get.Type eq 'M10'}اشتراکی{else}اختصاصی{/if}</li>
            </ol>
        </div>
                        ...
                        {foreach key=key item=item from=$Ticket}
                            {foreach key=number item=Rout from=$item.OutputRoutes}
                                ...
                                <tr id="flight-{$Rout.FlightNo}" {if $bgColor neq ''}style="background-color: {$bgColor}"{/if}>
                                    ...
                                    <td class="align-middle font20">{$objFunctions->AirlineName($Rout.Airline.Code)}</td>
                                    <td class="align-middle font20">{$Rout.CabinType}</td>
                                    <td class="align-middle font20">{($item.PassengerDatas[0].BasePrice) * 10|number_format}</td>
