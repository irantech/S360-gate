            ...
            {if $TicketRevalidate.message eq ''}
                <div class="white-box">
                    ...
                    <form id="SearchTicketHistory" method="post"
                          action="{$smarty.const.rootAddress}repeatStepBookAndReserveSourceFour">
                        <input type="hidden" name="RequestNumber" id="RequestNumber"
                               value="{$TicketRevalidate.RequestNumberOfReserve}">
                        <input type="hidden" name="RequestNumberOfTicketOld" id="RequestNumberOfTicketOld"
                               value="{$TicketRevalidate.RequestNumberOfTicketOld}">
                        {foreach key=key item=item from=$TicketRevalidate.UserInfo}
                            ...
                            <hr>
                        {/foreach}
                        {if $smarty.get.SourceId eq '8'}
                        <div class="form-group col-lg-3 col-md-3 col-xs-12 col-sm-12 ">
                            <div class="div-captcha">
                                <img src="{$TicketRevalidate.CaptchaCode}" name="LinkCaptchaImg" id="LinkCaptchaImg">
                                <input id="LinkCaptcha" type="text" placeholder="کد امنیتی را وارد کنید" name="LinkCaptcha">
                            </div>
                            <input type="hidden" name="api_id" id="api_id" value="8">
                            {else}
                            <input type="hidden" name="api_id" id="api_id" value="11">
                            {/if}
                        <div class="row">
