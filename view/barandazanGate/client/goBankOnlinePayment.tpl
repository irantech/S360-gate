        ...
        <div class="main-bank-box">
            ...
            {if $smarty.post.bankType neq 'credit'}
                ...
                <div class="txtAlertBank">
                    ##onlinepaymentbankredirectedagain##
                </div>
            {/if}
        </div>
    </div>
...
{if $smarty.post.bankType neq 'credit'}
    {$objOnlinePayment->setPortBank($smarty.post.bankType, $smarty.post.requestNumber)}
{/if}
...
{$objBank->calculateAmount('onlinePayment', $smarty.post.factorNumber, $smarty.post.price)}
{$objBank->executeBank('go')}

{if $objBank->failMessage eq ''}
    {$objOnlinePayment->sendUserToBank($smarty.post.factorNumber)}
{else}
