{load_presentation_object filename="installmentCalculator" assign="installmentCalculator"}
{assign var="result" value=$installmentCalculator->getData()}
            ...
            <div class='installmentCalculator_box-parent'>
                <div class="col-lg-6 col-12">
                    <div class="installmentCalculator_box_div1">
                        <input type="hidden" id='initial_payment' name='initial_payment' value='{$result['initial_payment']}'>
                        <input type="hidden" id='profit_percentage' name='profit_percentage' value='{$result['profit_percentage']}'>
                        <div>
                            <div class="header">
                                <h3>تعداد اقساط</h3>
                                <div class="div-rangeslider">
                                    <h6>{$result['min_installments']}</h6>
                                    <span>قسط</span>
                                </div>
                            </div>
                            <div dir="ltr">
                                <input  onchange="getInfoCalculator()" id='installments' data-rangeslider="" max="{$result['max_installments']}" min="{$result['min_installments']}" type="range" value="0"/>
                            </div>
                        </div>
                        <div>
                            <div class="header">
                                <h3>مبلغ حدودی هر {$result['title']} </h3>
                                <div class="div-rangeslider2">
                                    <h6>{$result['min_price']}</h6>
                                    <span>میلیون</span>
                                </div>
                            </div>
                            <div dir="ltr">
                                <input onchange="getInfoCalculator()" id='price' data-rangeslider2="" max="{$result['max_price']}" min="{$result['min_price']}" step="10" type="range" value="0"/>
                            </div>
                        </div>
                        <div>
                            <div class="header">
                                <h3> پیش پرداخت دلخواه</h3>
                                <div class="div-rangeslider3">
                                    <h6>{$result['initial_payment']}</h6>
                                    <span>درصد</span>
                                </div>
                            </div>
                            <div dir="ltr">
                                <input onchange="getInfoCalculator()" id='initial_payment_range' data-rangeslider3="" max="90" min="{$result['initial_payment']}" step="5" type="range" value="0"/>
                            </div>
                        </div>
                    </div>
                </div>
