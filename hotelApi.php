...
class hotelApi {
	...
	public function __construct() {
		...
		//		$isTestApi      = ( $parseUrl[2] == 'apiHotelsTest' ) ? true : false;
		$method         = $parseUrl[3];
		//		if ( ! $isTestApi ) {
		//			$this->showJson( [
		//				'Success' => false,
	...
	private function setApiRoot() {
		if ( ( isset( $_SERVER["HTTP_HOST"] ) && strpos( $_SERVER["HTTP_HOST"], '192.168.1.100' ) !== false ) || ( isset( $_SERVER["HTTP_HOST"] ) && strpos( $_SERVER["HTTP_HOST"], 'online.1011.ir' ) !== false ) || ( isset( $_SERVER["HTTP_HOST"] ) && strpos( $_SERVER["HTTP_HOST"], '1011.ir' ) !== false ) || ( isset( $_SERVER["HTTP_HOST"] ) && strpos( $_SERVER["HTTP_HOST"], 'localhost' ) !== false ) ) {
			$this->apiRoot = 'http://safar360.com/CoreTestDeveloper/V-1/Hotels/';
			//			$this->apiRoot = 'http://192.168.1.100/CoreTestDeveloper/V-1/Hotels/';
		} else {
	...
	private function getAuthUsername( $username = null, $password = null ) {
		$Sql = "SELECT AUTH.Username FROM client_auth_tb AS AUTH
				LEFT JOIN client_user_api AS API ON AUTH.ClientId = API.clientId
				WHERE API.userName='{$username}' AND API.keyTabdol='{$password}'
				AND AUTH.ClientId=API.clientId AND ((SourceId='6' AND serviceId='9') OR (SourceId='12' AND serviceId='10'))";
	...
	protected function CurlGet( $apiRoute = '', $data = [] ) {
		...
		if ( ! $apiResult ) {
			...
			$result['Result']  = [$apiResult,$this->apiRoute];
	...
	public function hotelList( $data ) {
		...
		if ( ! isset( $data['Rooms'] ) || $data['Rooms'] < 1 ) {
	...
	public function Book( $data ) {
		...
		if ( count( $Rooms ) != count( $Passengers ) ) {
			$this->showError( 400, 'اطلاعات وارد شده صحیح نیست', 'BK-005', 'Please Post Each room a passenger', 'لطفا برای هر اتاق یک سرگروه ارسال کنید' );
		}
		$apiResult = $this->processRequest( 'post', 'Book', $data, true );
		if ( $apiResult['Success'] != true || $apiResult['StatusCode'] != 200 ) {
		...
		$updateBook['irantech_commission'] = $it_commission;
		...
		foreach ( $apiResultReservation as $key => $item ) {
			$roomIndex = array_keys( $Rooms[ $key ] )[0];
			$roomCount = array_values( $Rooms[ $key ] )[0];
			$prices    = $item['Room']['Prices'];
			...
			$dataInsert['room_id']                 = $item['Room']['RoomIndex'];
			...
			$dataInsert['total_price']  = $item['Room']['TotalPrices']['Board'];
	...
	public function Reserve( $data ) {
		...
		if ( $check['status'] == 'FALSE' ) {
			$this->showError( 402, 'عدم اعتبار کافی', 'BK-005', 'not enough credit', 'اعتبار شما برای این خرید کافی نیست' );
		}
	...
	public function setCreditToSuccess( $factorNumber, $bankTrackingCode, $clientId ) {
		...
		return $admin->ConectDbClient( '', $clientId, 'Update', $data, 'transaction_tb', $condition );
	}
}

if ( ! function_exists( 'hotelApi' ) ) {
	function hotelApi() {
		return new hotelApi();
	}
}
