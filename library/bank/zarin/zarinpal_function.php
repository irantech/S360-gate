...
class zarinpal extends BankBase
{

    private $is_sandbox=false;

    private $payment_url='https://www.zarinpal.com/pg/StartPay/';
    private $request_url='https://api.zarinpal.com/pg/v4/payment/request.json';
    private $verify_url='https://api.zarinpal.com/pg/v4/payment/verify.json';
    private $sandbox_payment_url='https://sandbox.zarinpal.com/pg/StartPay/';
    private $sandbox_request_url='https://sandbox.zarinpal.com/pg/rest/WebGate/PaymentRequest.json';
    private $sandbox_verify_url='https://sandbox.zarinpal.com/pg/rest/WebGate/PaymentVerification.json';
    ...
    public function error_message($code, $desc='', $cb='', $request = false)
    {
       /* if (empty($cb) && $request === true) {
        ...
        $error = array(
            ...
            "-9" => "The amount must be at least 1000.",
...
//		exit;
    ...
    public function requestPayment($request_data = [])
    {
        ...
        $url = $this->is_sandbox ? $this->sandbox_request_url : $this->request_url;
        $jsonData = json_encode($data);
        $ch = curl_init($url);
        curl_setopt($ch, CURLOPT_USERAGENT, 'ZarinPal Rest Api v1');
        curl_setopt($ch, CURLOPT_CUSTOMREQUEST, 'POST');
        ...
        if ($err) {
            return $this->showResult(false, $result, 'Error happened = ' . $err);
        } else {
            if (empty($result['errors'])) {
                if ($result['data']['code'] == 100) {
                    return $this->showResult(true, $result, 'go to payment page');
                } else {
                    return $this->showResult(false, $result, 'Error code = ' . $result['data']['code']);
                }
            } else {
                return $this->showResult(false, $result, $this->error_message($result['errors']['code']));
            }
        }
    }
    ...
    {
        return $this->is_sandbox ? $this->sandbox_payment_url : $this->payment_url;
    }
    ...
    public function verifyPayment($verify_params = [])
    {
        ...
        $data = array(
            "merchant_id" =>$verify_params['merchant_id'],
            "authority" => $verify_params['authority'],
            "amount" => $amount,
        );
        $url = $this->is_sandbox ? $this->sandbox_verify_url : $this->verify_url;
        $jsonData = json_encode($data);
        $ch = curl_init($url);
        ...
        if ($err) {
            ...
            if ($result['data']['code'] == 100) {
                return $this->showResult(true, $result, 'تراکنش انجام شد');
            } else {
                return $this->showResult(false, $result, $this->error_message($result['errors']['code']));
            }
        }
